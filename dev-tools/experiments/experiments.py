...
parser.add_argument('--spark_executor_num', type=str, help='Number of spark executors - input format "3,4,5" ' , default = "2,3")
parser.add_argument('--spark_executor_memory', type=str, help='Spark executors memory in Gb - input format "1,2,3" ', default = "1")
parser.add_argument('--dataset_size', type=str, help='Dataset size - input format "100,1000,3000000" ', default = "100")

# Service arguments
parser.add_argument('--spark_master', type=str, help='Type of Spark master - k8s or local (default local[1]) ', default = "local[1]") #default = "k8s://https://192.168.137.152:8443"
parser.add_argument('--use_state_file', type=str, help='Usage of state file - [use|ignore] ', default = "ignore")

parser.add_argument('--spark_kubernetes_authenticate_serviceAccountName', type=str, help='K8s service account ', default = "spark")
parser.add_argument('--spark_kubernetes_namespace', type=str, help='K8s namespace ', default = "default")
parser.add_argument('--spark_kubernetes_container_image', type=str, help='Container image for K8s pod ', default = "local/spark-lama")
    ...
    with Pool(processes = len(arg_product)) as pool:
        ...
        if arg_space.use_state_file in ('use'):
            ...
            if os.path.exists('./dev-tools/experiments/experiment.log'):
                with open(f'./dev-tools/experiments/experiment.log', "r") as file:
        ...
        for item in arg_product:
            ...
            for key in vars(arg_space):
                if key not in ("appName", "spark_executor_instances", "spark_executor_memory", "spark_master", "dataset_size", "use_state_file"):
            ...
            session_args.append([experiment_name,
                                int(item[2]),  # Data size                          
                                spark_args,
                                arg_space.spark_master
                                ])
